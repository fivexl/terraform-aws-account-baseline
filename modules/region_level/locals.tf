locals {
  default_s3_tf_state_bucket_name = "terraform-state-${sha1(format("%s-%s", data.aws_caller_identity.current.account_id, data.aws_region.current.region))}"
  state_bucket_name               = var.s3_tf_state_bucket_name != "" ? var.s3_tf_state_bucket_name : local.default_s3_tf_state_bucket_name

  default_s3_access_logs_bucket_name = "access-logs-${sha1(format("%s-%s", data.aws_caller_identity.current.account_id, data.aws_region.current.region))}"
  ...
  default_cmk_access_logs_bucket_name = "cmk-access-logs-${sha1(format("%s-%s", data.aws_caller_identity.current.account_id, data.aws_region.current.region))}"
