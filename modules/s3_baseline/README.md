...
| <a name="input_server_side_encryption_configuration"></a> [server\_side\_encryption\_configuration](#input\_server\_side\_encryption\_configuration) | Server-side encryption for the bucket. Defaults match AWS/provider normalization (AES256, block SSE-C, no S3 bucket key) to avoid perpetual drift. | `any` | <pre>{<br/>  "rule": {<br/>    "apply_server_side_encryption_by_default": {<br/>      "sse_algorithm": "AES256"<br/>    },<br/>    "blocked_encryption_types": [<br/>      "SSE-C"<br/>    ],<br/>    "bucket_key_enabled": false<br/>  }<br/>}</pre> | no |
